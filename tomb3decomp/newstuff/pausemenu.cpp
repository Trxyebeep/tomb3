...
static long ShowPauseRequester(const char* txt, const char* opt1, const char* opt2, ushort selected)
{
	...
	SetRequesterHeading(&Pause_Requester, txt, 0, 0, 0);
	AddRequesterItem(&Pause_Requester, opt1, 0, 0, 0);
	AddRequesterItem(&Pause_Requester, opt2, 0, 0, 0);
	inputDB = 0;
	input = 0;
	mode = 1;
	return -1;
